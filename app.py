...
with tab1:
    ...
    with get_conn() as conn:
        ...
        att_id_for_edit = colB.number_input(
            "Attendance ID to Edit/Delete", value=1, step=1
        )
        if colB.button("Edit Attendance"):
            edit_attendance(
                conn,
                attendance_id=int(att_id_for_edit),
                new_status=status,
                user_id=int(user_id),
                ip_address=ip,
                notes=notes,
    ...
    with get_conn() as conn:
        if colU.button("Attempt UPDATE audit_logs"):
            try:
                attempt_modify_audit_logs(
                    conn, operation="UPDATE", user_id=int(user_id), ip_address=ip
                )
        ...
        if colD.button("Attempt DELETE audit_logs"):
            try:
                attempt_modify_audit_logs(
                    conn, operation="DELETE", user_id=int(user_id), ip_address=ip
                )
            except Exception as e:
                st.error(f"Blocked: {e}")
...
with tab2:
    ...
    with get_conn() as conn:
        rows = fetch_logs(
            ...
            user_id=user_filter_val,
            course_id=course_filter_val,
        )
    ...
    if col2.download_button(
        "Export PDF",
        data=to_pdf_bytes("SAMS2 Audit Logs", rows),
        file_name="audit_logs.pdf",
