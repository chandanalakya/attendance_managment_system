...
def find_user_by_email(email):
    ...
    cur.execute("SELECT * FROM users WHERE email=%s AND role='faculty'", (email,))
    user = cur.fetchone()
    cur.close()
    conn.close()
    return user
...
def create_user(email, password):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO users (email, password_hash, role, is_approved) VALUES (%s,%s,'faculty',0)",
        (email, pw_hash)
    )
    conn.commit()
    cur.close()
...
# -----------------------
    ...
    if not st.session_state.user:
        ...
        if details:
            ...
            if submit_details:
                save_faculty_details(st.session_state.user["id"], full_name, faculty_id, department, course, designation)
